...
#define GREEN         0x00FF00       // Turn strip on with GREEN color
#define BLUE          0x0000FF       // Turn strip on with BLUE color
#define JUST_ON       0
#define CHASE         1
#define FLASH         2
#define THEATER       3
  ...
  if (rf12_recvDone()) {
    ...
    if(rf12_data[0] == NODE_NUMBER) {
      switch(leds_mode)
      {
        ...
        case FLASH:
        Serial.print("Starting FLASH show for ");
        Serial.print(MINUTES);
        Serial.println(" minutes");
        leds_mode++;
        flash(NODE_COLOR);
        ...
        case THEATER:
        Serial.print("Starting THEATER show for ");
        Serial.print(MINUTES);
        Serial.println(" minutes");
        leds_mode++;
        theaterChase(NODE_COLOR);
        break;
...
static void chase(uint32_t c) {
  for(uint16_t i = 0; i < PULSES/2; i++) {
    for(uint16_t j = 0; j < strip.numPixels() + N_LEDS; j++) {
      strip.setPixelColor(j, c); // Draw new pixel
      strip.setPixelColor(j - N_LEDS, 0); // Erase pixel a few steps back
      strip.show();
      delay(25); // ~ 2s
    }
  }
...
static void theaterChase(uint32_t c) {
  for (int j = 0; j < PULSES * 13; j++) {  //do 10 cycles of chasing
    for (int q = 0; q < 3; q++) {
